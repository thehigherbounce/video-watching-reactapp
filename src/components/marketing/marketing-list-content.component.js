...
export default class MarketingListContent extends Component {
  ...
  editGamePage(index) {
    if (this.props.details[index].status === 'Pending' || this.props.details[index].status === 'pending') {
      this.toastBC.show({severity:'error', summary: 'Status Error', detail:'Pending game cannot be edited!', life: 3000});
    } else {
    ...
    if (this.state.redirect) {
      return <Redirect
        to={{
          ...
          state: {index: this.state.editIndex, id: this.state.editData.id}
        }}
      />
    }
    return (
